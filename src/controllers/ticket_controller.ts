...
export const buy_ticket: AuthenticatedRoutesHandler<{}, { place_id: number }> = async (req, res) => {
    ...
    try {
        ...
        if (!reservation || reservation != user_id) return res.status(404).json({ message: "Reserva expirada ou inválida." })
...
const handleError = async (res: Response, error: unknown, place_id: number) => {
